...
import {getPopularTopics, searchByTopic} from '../../../utils/storage/sql';
...
import {getTopicTypeLabel} from '../../../utils/utils/utils';
...
const SearchPage = ({navigation}: {navigation: any}) => {
  ...
  const getPopular = async (): Promise<void> => {
    const topics: Topic[] = await getPopularTopics(contentLanguage);
  ...
  const saveRecents = async (newRecentsArray: Topic[]) => {
    await setStorageRecent(newRecentsArray, contentLanguage);
    if (user) {
  ...
  const getRecents = async () => {
    const retrievedArray = await getStorageRecents(contentLanguage);
  ...
  const executeSearch = async (param: string): Promise<void> => {
    ...
    const topics: Topic[] = await searchByTopic(param, contentLanguage);
    setItems(topics);
  };
