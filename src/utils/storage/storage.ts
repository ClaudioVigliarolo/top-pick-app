...
import {DEFAULT_LANGUAGE} from '../../context/translations';
...
export const setStorageAppLanguage = async (language: Lang): Promise<void> => {
  try {
...
export const setStorageContentLanguage = async (
  language: Lang,
): Promise<void> => {
  try {
    await AsyncStorage.setItem(keys.CONTENT_LANGUAGE, language);
  } catch (error) {
    // Error saving data
  }
};

export const getStorageAppLanguage = async (): Promise<Lang | null> => {
  try {
    const retrievedLanguage = await AsyncStorage.getItem(keys.APP_LANGUAGE);
...
export const getStorageContentLanguage = async (): Promise<Lang> => {
  try {
    const retrievedLanguage = await AsyncStorage.getItem(keys.CONTENT_LANGUAGE);
    if (retrievedLanguage === null) {
      return DEFAULT_LANGUAGE;
    } else {
      return retrievedLanguage as Lang;
...
export const setAppContentUpdated = async (val: boolean): Promise<void> => {
  AsyncStorage.setItem(keys.CONTENT_LANGUAGE, JSON.stringify(val));
};
export const getStoredLanguage = async (): Promise<Lang> => {
  try {
    const currentLanguage = await AsyncStorage.getItem(keys.CONTENT_LANGUAGE);
...
export const loadSettings = async (settings: UserSettings): Promise<void> => {
  ...
  await setStorageAppLanguage(settings.appLanguage);
  await setStorageContentLanguage(settings.contentLanguage);
...
export const getStoreSettings = async (): Promise<UserSettings> => {
  const settings: UserSettings = {
    ...
    appLanguage: (await getStorageAppLanguage()) as Lang,
    contentLanguage: await getStorageContentLanguage(),
  };
  return settings;
};
