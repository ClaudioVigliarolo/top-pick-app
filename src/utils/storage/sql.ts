...
const DB = SQLite.openDatabase(
  ...
  async () => {
    ...
    console.log('vers', version, currVersion);
    if (version < currVersion) {
      //Call upgrade scripts
      console.log('ok updating');
      upgradeDB(version, currVersion);
    }
  },
  () => {},
);
...
export const upgradeDB = async (oldVersion: number, newVersion: number) => {
  ...
  statements.push(
    `UPDATE "version" SET version = ${newVersion} WHERE "version" = ${oldVersion};`,
  );
  ...
  //ok now after having updated up to the current version, now update also version in db
};
    ...
    await DB.transaction((tx) => {
      topics.forEach((el: Topic, i: number) => {
        ...
        if (parameters.length >= SQL_MAX_TUPLES || i == topics.length - 1) {
          tx.executeSql(
            INSERT_QUERY +
              'INTO topics  (id, ref_id, title, source, timestamp, type, user_modified, level, description, image, lang) VALUES ' +
              bigqery +
              ';',
            parameters,
            (tx, results) => {
    ...
    await DB.transaction((tx) => {
      questions.forEach((el: Question, i: number) => {
        ...
        if (parameters.length >= SQL_MAX_TUPLES || i == questions.length - 1) {
          tx.executeSql(
            INSERT_QUERY +
              'INTO questions  (id, topic_id, title, n, liked, description, link, user_modified, lang) VALUES ' +
              bigqery +
              ';',
            parameters,
            (tx, results) => {
